# 👋 Backend Engineer (PHP 10+ yrs) — Builder · Commerce · Core Logic

저는 **PHP 백엔드**에서 10년 넘게 서비스의 “시작 → 성장” 구간을 함께 지나왔습니다.  
프레임워크를 *그냥 쓰는* 수준을 넘어, **쇼핑몰 빌더 / CMS / 템플릿 엔진**처럼 비즈니스의 뼈대가 되는 코어를 **직접 설계하고 구현**해 온 경험이 강점입니다.

> “돌아가는 코드”보다 **유지보수**와 **성능**, 그리고 팀/비즈니스에 남는 **실질적인 결과물**을 더 중요하게 생각합니다.

---

## ✅ What I Do Well

- **Core 설계/구현**: 쇼핑몰 빌더, CMS, 템플릿 엔진 등 핵심 로직 설계 및 개발
- **레거시 개선**: 유지보수 효율을 높이는 리팩토링 및 구조 정리(MVC 등)
- **운영 감각**: Linux 서버 환경, DB 운영/튜닝이 일상처럼 익숙함
- **협업**: GitHub/GitLab, Jira 기반으로 팀 개발 프로세스에 익숙

---

## 🔭 Now (확장 중인 영역)

익숙함에 머무르지 않기 위해, 최근에는 아래를 “깊게” 학습/적용 중입니다.

- **Python (FastAPI)**: 현대적인 API 설계/구조를 더 탄탄하게
- **NestJS (TypeScript)**: 명확한 계층/도메인 분리와 유지보수성 높은 서버 구조 탐색

---

## 🧩 Selected Experience

### 🏗 Clickn — Homepage & Shopping Mall Builder
- **Role**: Backend Lead Developer  
- **Goal**: 누구나 쉽게 쇼핑몰을 구축할 수 있는 빌더 서비스 개발
- **What I solved**
  - 기존 빌더의 **복잡한 UI / 느린 템플릿 로딩** 문제
- **Stack**: PHP · CodeIgniter · MariaDB · jQuery

---

### 🛒 Firstmall — Large-scale Commerce Solution
- **Role**: Core Developer  
- **What I did**
  - 결제 모듈 연동 및 **정산 시스템 로직 최적화**
  - 레거시를 현대적인 **MVC 패턴 기반으로 리팩토링** → 유지보수 효율 증대

---

### 🧱 Ypage & DanggunWeb — Website Auto Builder
- **Role**: Full Stack Developer  
- **What I did**
  - 중소기업용 **홈페이지 자동 생성 빌더** 개발/유지보수
  - 고객사별 커스텀 요구사항 대응
  - **DB 스키마 설계 최적화**

---

## 🛠 Tech Stack

### Working Level (실무 즉시 투입)
**Core Backend Engineering**  
![PHP](https://img.shields.io/badge/PHP-777BB4?logo=php&logoColor=white&style=flat-square)
![Laravel](https://img.shields.io/badge/Laravel-FF2D20?logo=laravel&logoColor=white&style=flat-square)
![CodeIgniter](https://img.shields.io/badge/CodeIgniter-EF4223?logo=codeigniter&logoColor=white&style=flat-square)
![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white&style=flat-square)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?logo=fastapi&logoColor=white&style=flat-square)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?logo=typescript&logoColor=white&style=flat-square)
![NestJS](https://img.shields.io/badge/NestJS-E0234E?logo=nestjs&logoColor=white&style=flat-square)
![Node.js](https://img.shields.io/badge/Node.js-339933?logo=node.js&logoColor=white&style=flat-square)

**Database & Data Modeling**  
![MySQL](https://img.shields.io/badge/MySQL-4479A1?logo=mysql&logoColor=white&style=flat-square)
![MariaDB](https://img.shields.io/badge/MariaDB-003545?logo=mariadb&logoColor=white&style=flat-square)
![Oracle](https://img.shields.io/badge/Oracle-F80000?logo=oracle&logoColor=white&style=flat-square)
![SQLAlchemy](https://img.shields.io/badge/SQLAlchemy-D71F00?logo=sqlalchemy&logoColor=white&style=flat-square)

**Infrastructure & Runtime**  
![Linux](https://img.shields.io/badge/Linux-FCC624?logo=linux&logoColor=black&style=flat-square)
![NGINX](https://img.shields.io/badge/NGINX-009639?logo=nginx&logoColor=white&style=flat-square)
![Apache](https://img.shields.io/badge/Apache-D22128?logo=apache&logoColor=white&style=flat-square)
![Docker](https://img.shields.io/badge/Docker-2496ED?logo=docker&logoColor=white&style=flat-square)

**Frontend Integration**  
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?logo=javascript&logoColor=black&style=flat-square)
![jQuery](https://img.shields.io/badge/jQuery-0769AD?logo=jquery&logoColor=white&style=flat-square)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?logo=html5&logoColor=white&style=flat-square)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?logo=css3&logoColor=white&style=flat-square)
![Bootstrap](https://img.shields.io/badge/Bootstrap-7952B3?logo=bootstrap&logoColor=white&style=flat-square)
![React](https://img.shields.io/badge/React-61DAFB?logo=react&logoColor=black&style=flat-square)

**Dev Environment & Collaboration**  
![Git](https://img.shields.io/badge/Git-F05032?logo=git&logoColor=white&style=flat-square)
![GitHub](https://img.shields.io/badge/GitHub-181717?logo=github&logoColor=white&style=flat-square)
![GitLab](https://img.shields.io/badge/GitLab-FC6D26?logo=gitlab&logoColor=white&style=flat-square)
![Jira](https://img.shields.io/badge/Jira-0052CC?logo=jira&logoColor=white&style=flat-square)
![VSCode](https://img.shields.io/badge/VSCode-007ACC?logo=visualstudiocode&logoColor=white&style=flat-square)
![WSL2](https://img.shields.io/badge/WSL2-4D4D4D?logo=windows-terminal&logoColor=white&style=flat-square)
![DBeaver](https://custom-icon-badges.demolab.com/badge/-DBeaver-372923?logo=dbeaver-mono&logoColor=white&style=flat-square)

---

## 📚 Studying (개인 프로젝트/학습)

![React](https://img.shields.io/badge/React-61DAFB?logo=React&logoColor=black&style=flat-square)
![Oracle](https://img.shields.io/badge/Oracle-F80000?logo=Oracle&logoColor=white&style=flat-square)
![Android%20Studio](https://img.shields.io/badge/Android%20Studio-3DDC84?logo=Android%20Studio&logoColor=white&style=flat-square)
![JAVA](https://custom-icon-badges.demolab.com/badge/Java-007396.svg?logo=java&logoColor=white&style=flat-square)
![Spring Boot](https://img.shields.io/badge/Spring%20Boot-6DB33F?logo=Spring%20Boot&logoColor=white&style=flat-square)

---

## 🎯 I Care About
- 유지보수하기 쉬운 구조
- 성능이 확실한 개선
- 팀과 비즈니스에 “남는” 결과
